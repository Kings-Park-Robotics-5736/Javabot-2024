...
public class TrajectoryCommandsFactory {
    ...
    /**
     * @brief Generates a command that will follow an autonomous path group with events
     * @param robotDrive subsystem
     * @param intake     subsystem
     * @param escalator  subsystem
     * @return
     */
    public static Command generateAutoTrajectoryCommand(DriveSubsystem robotDrive, IntakeSubsystem intake, EscalatorAssemblySubsystem escalator){

        // Load the path group from the path planner. This is a list of trajectories that will be run in sequence.
        List<PathPlannerTrajectory> pathGroup = PathPlanner.loadPathGroup("main", new PathConstraints(1, 1));
        ...
        SwerveAutoBuilder autoBuilder = new SwerveAutoBuilder(
            ...
            robotDrive // The drive subsystem. Used to properly set the requirements of path following commands
        );
    ...
    /**
     * @brief Generates a command that will follow a path, with a start and end point
     * @param robotDrive subsystem
    ...
    /**
     * @brief Generates a command that will follow a path, with a start and end point, and an option to reset odometry if first path
     * @param robotDrive
     * @param start
     * @param end
     * @param isFirstPath whether or not this is the first time and to reset odometry 
     * @return
     */
    public static Command generateTrajectoryCommand(DriveSubsystem robotDrive, Pose2d start, Pose2d end, Boolean isFirstPath) {
    ...
    /**
     * @brief Generates a command that will follow a path, with a start and end point, and intermediate points
     * @param robotDrive
     * @param start
     * @param points points in between start and end. These are used to generate a smooth path
     * @param end
     * @param isFirstPath
     * @return
     */
    public static Command generateTrajectoryCommand(DriveSubsystem robotDrive, Pose2d start, List<PathPoint> points, Pose2d end, Boolean isFirstPath) {
        ...
        if(points.size() > 0){
            ...
            traj1 = PathPlanner.generatePath(
                ...
                new PathPoint(start.getTranslation(), start.getRotation(), start.getRotation()), // position, heading
                points.get(0),
     ...
     * @return
     */
    public static Command generateTrajectoryCommand(DriveSubsystem robotDrive, PathPlannerTrajectory traj1, Boolean isFirstPath) {
